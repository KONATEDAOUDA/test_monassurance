    ...
    slimScroll: function(){
      ...
      if( $slimScrollEl.length > 0 ){
        $slimScrollEl.each(function() {
          ...
          element.slimScroll({height: '100%',start: 'bottom'});
        });
      }
    ...
    daterangePicker: function() {
      ...
      if ($pickDateEl.length > 0) {
        $pickDateEl.each(function() {
          ...
          s = window.location.search.match(/(\?|&)start\=([^&]*)/)
          e = window.location.search.match(/(\?|&)end\=([^&]*)/)

          start_get = (s==null) ? moment().startOf('years') : decodeURIComponent(s[2])
          end_get = (e==null) ? moment().endOf('years') : decodeURIComponent(e[2])
          
          element.find('span').html(moment().startOf('year').format('D MMMM YYYY') + ' - ' + moment().endOf('month').format('D MMMM YYYY'));
          ...
          element.daterangepicker({
            ...
            ranges: {
              ...
              'Cette année': [moment().startOf('years'), moment().endOf('month')]
            },
            opens: 'left',
            drops: 'down',
            ...
            locale: {
              ...
              monthNames: ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juiller', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'],
              firstDay: 1
            }
          }, function(start, end, label) {
            element.find('span').html(start.format('D MMMM YYYY') + ' - ' + end.format('D MMMM YYYY'));
            $('#start').val(start.toISOString().substring(0, 10));
            $('#end').val(end.toISOString().substring(0, 10));
            $('#label').val(label);
            var getUrl = window.location.href;
            if((getUrl.indexOf('dashboard') !== -1) || (getUrl.indexOf('stats') !== -1)) $('#custom_search').submit();
              $.ajax({
                url: $('#custom_search').attr('action'),
                type : $('#custom_search').attr('method'),
                data : $('#custom_search').serialize(),
