(function () {
		...
		jQuery("#callMeForm").submit(function (e){   
		  event.preventDefault();  

		  var valid = true;
		    jQuery('.call-me').each(function(i, el){
		      if(valid && $(el).val()=='' ) valid = false; 
		    })

		  if(valid){
		    jQuery("#callLoader").show()
		    setTimeout(function() {
		      $.ajax({
		        url: $("#callMeForm").attr('action'),
		        type: $("#callMeForm").attr('method'),
		...
		jQuery(window).load(function() { // makes sure the whole site is loaded
			jQuery('#loader').delay(10).fadeOut('slow'); // will fade out the white DIV that covers the website.
		...
		$(window).load(function() {
			...
			var $container2 = $('#cases-container2');
			$container.isotope({
				filter: '.contrats',
				animationOptions: {
					duration: 750,
