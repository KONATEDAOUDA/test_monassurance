...
namespace App\Http\Controllers\Backoffice\Quotation;
...
{
    Log::info('Début de caculAutoQuotationFromDb.', [
        'autoInfo_ID' => $autoInfo_ID,
        'user_ID' => $user_ID,
        'assurance_ID' => $assurance_ID,
    ]);

    // Vérifier que autoInfo_ID n'est pas null
    if (empty($autoInfo_ID)) {
        Log::error('autoInfo_ID est null.');
        throw new \Exception('autoInfo_ID est null. Le calcul du devis ne peut pas être effectué.');
    }

    // Récupérer autoInfos
    Log::info('Récupération de autoInfos avec l\'ID : ' . $autoInfo_ID);
    $autoInfos = autoInfos::where('id', $autoInfo_ID)->first();

    if (!$autoInfos) {
        Log::error('autoInfos introuvable avec l\'ID : ' . $autoInfo_ID);
        throw new \Exception('autoInfos introuvable.');
    }

    Log::info('autoInfos trouvé :', ['autoInfos' => $autoInfos]);

    // Vérifier que user_ID n'est pas null
    if (empty($user_ID)) {
        Log::error('user_ID est null.');
        throw new \Exception('user_ID est null. Le calcul du devis ne peut pas être effectué.');
    }

    // Récupérer userInfos
    Log::info('Récupération de userInfos avec l\'ID : ' . $user_ID);
    $userInfos = User::where('id', $user_ID)->first();

    if (!$userInfos) {
        Log::error('userInfos introuvable avec l\'ID : ' . $user_ID);
        throw new \Exception('userInfos introuvable.');
    }

    Log::info('userInfos trouvé :', ['userInfos' => $userInfos]);

    // Vérifier que assurance_ID n'est pas null
    if (empty($assurance_ID)) {
        Log::error('assurance_ID est null.');
        throw new \Exception('assurance_ID est null. Le calcul du devis ne peut pas être effectué.');
    }

    // Récupérer assuranceInfos
    Log::info('Récupération de assuranceInfos avec l\'ID : ' . $assurance_ID);
    $assuranceInfos = AssuranceAutoInfos::where('id', $assurance_ID)->first();

    if (!$assuranceInfos) {
        Log::error('assuranceInfos introuvable avec l\'ID : ' . $assurance_ID);
        throw new \Exception('assuranceInfos introuvable.');
    }

    Log::info('assuranceInfos trouvé :', ['assuranceInfos' => $assuranceInfos]);
    ...
    $quote = DB::table('quotation')
        ->where([
            'assurance_infos_id' => $assurance_ID,
            'user_id' => $user_ID,
            'product_id' => $autoInfo_ID,
        ])
        ->first();

    if (!$quote) {
        Log::error('Devis introuvable.');
        throw new \Exception('Devis introuvable.');
    }

    // Accéder aux propriétés de autoInfos
    $matriculation = $autoInfos->immatriculation ?? null;
    $model_id = $autoInfos->model_id ?? null;
    $category_id = $autoInfos->category ?? null;
    $zone = $autoInfos->parkingzone ?? null;
    $energy = $autoInfos->energy ?? null;
    $power = $autoInfos->power ?? null;
    $cu = $autoInfos->charge_utile ?? null;
    $cylindree = $autoInfos->cylindree ?? null;
    $type_id = $autoInfos->type_id ?? null;
    $firstrelease = $autoInfos->firstrelease ?? null;
    $placesnumber = $autoInfos->placesnumber ?? null;
    $vneuve = $autoInfos->vneuve ?? null;
    $vvenale = $autoInfos->vvenale ?? null;

    // Accéder aux propriétés de userInfos
    $firstname = $userInfos->firstname ?? null;
    $lastname = $userInfos->lastname ?? null;
    $email = $userInfos->email ?? null;
    $phone = $userInfos->phone ?? null;
    $date_perm = $userInfos->date_pc ?? null;

    // Accéder aux propriétés de assuranceInfos
    $periode_id = $assuranceInfos->periode ?? null;
    $datepriseeffet = $assuranceInfos->datepriseeffet ?? null;
    $type_subscription = $assuranceInfos->subscription_type ?? null;
    $selectedformule = $assuranceInfos->guarante ?? null;

    // Formater les garanties sélectionnées
    $selected_guaranties_array = $this->format_to_array($selectedformule);

    // Définir la compagnie préférée
    $pref_comp = $_GET['idcomp'] ?? $quote->company_id;
    ...
    // Calculer les coûts réglementaires
    ...
    // Initialiser les arguments pour le calcul
    $all_quotation = [];
    if ($periode_id == 1) {
    ...
    // Parcourir la liste des compagnies
    foreach ($this->getCompagniesList($pref_comp, $quote->company_id) as $company) {
        ...
        $all_quotation = $this->setGuarantiesCoast($rc, $periode_id, $date_perm, $phone, $company, $all_quotation, $type_subscription, $selectedformule, $selected_guaranties_array, $category_id, $args);
    ...
    // Trier les devis par montant TTC
    $qu_sorted = collect($all_quotation)->sortBy("TTC");

    // Retourner les devis triés
    if ($pref_comp == 0) {
